...
export default async function FlowPage({ params }: Props) {
  ...
  const { data: flow } = await admin.from('flows').select('*').eq('id', id).eq('is_active', true).single()
  if (!flow) notFound()

  if (profile?.role !== 'admin') {
    const { data: access } = await admin.from('flow_access').select('id').eq('flow_id', id).eq('profile_id', user.id).single()
    if (!access) notFound()
  }
  ...
  return (
    <div className="p-4 md:p-8 max-w-2xl mx-auto">
      ...
      <div className="flex items-start gap-3 md:gap-4 mb-6 md:mb-8">
        <div className="w-10 h-10 md:w-12 md:h-12 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center shrink-0">
          <FlowIcon icon={flow.icon} className="w-4 h-4 md:w-5 md:h-5 text-white" />
        </div>
