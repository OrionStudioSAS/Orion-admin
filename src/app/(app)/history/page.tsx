...
export default async function HistoryPage() {
  ...
  if (profile?.role !== 'admin') query = query.eq('profile_id', user.id)
      ...
      {!executions || executions.length === 0 ? (
        ...
        <div className="space-y-2">
          ...
          {executions.map((exec: any) => {
            ...
            return (
              <div key={exec.id} className="flex items-center gap-3 md:gap-4 bg-[#0f0f0f] border border-[#1e1e1e] rounded-xl px-3 md:px-5 py-3 md:py-4 hover:border-white/10 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-white truncate">{flow?.name || 'Flow supprimé'}</div>
                  {flow?.category && (
