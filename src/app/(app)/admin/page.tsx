  ...
  const sections = [
    {
      ...
      description: 'Créer des comptes, définir les rôles, assigner les flows aux clients.',
    },
    {
      href: '/admin/flows',
      icon: BoltIcon,
      title: 'Gestion des flows',
      description: 'Ajouter, éditer ou désactiver des flows N8N. Configurer les webhooks et les formulaires.',
    },
  ]
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      <div className="mb-10">
        ...
        <h1 className="text-3xl font-semibold text-white">Panneau d'admin</h1>
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        {stats.map(({ label, value, icon: Icon, href }) => (
          <Link
            key={label}
            href={href}
            className="group bg-[#0f0f0f] border border-[#1e1e1e] rounded-2xl p-5 hover:border-white/20 hover:bg-[#141414] transition-all"
          >
            <div className="flex items-center justify-between mb-3">
              <Icon className="w-4 h-4 text-[#71717a] group-hover:text-white transition-colors" />
              <ArrowRightIcon className="w-3.5 h-3.5 text-[#3f3f46] group-hover:text-[#71717a] transition-colors" />
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {sections.map(({ href, icon: Icon, title, description }) => (
          <Link
            key={href}
            href={href}
            className="group flex flex-col bg-[#0f0f0f] border border-[#1e1e1e] rounded-2xl p-6 hover:border-white/20 hover:bg-[#141414] transition-all"
          >
            ...
            <p className="text-[#71717a] text-xs leading-relaxed mb-5 flex-1">{description}</p>
